  ...
  return <Dialog.Root open={open} onOpenChange={requestOpen}>
    <Dialog.Trigger asChild>
      ...
      >
        ...
        <span className="truncate">Cài đặt LLM</span>
      </button>
    </Dialog.Trigger>
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 z-modal max-h-[90vh] w-[calc(100vw-2rem)] max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-panel bg-surface-strong p-5 shadow-floating focus:outline-none sm:p-6" aria-describedby="llm-settings-description">
        <div className="mb-5 pr-10">
          <Dialog.Title className="text-section-title text-text-primary">Nhà cung cấp LLM</Dialog.Title>
          <Dialog.Description id="llm-settings-description" className="mt-1 text-control text-text-muted">Cấu hình mô hình dùng cho tạo văn bản và tra cứu.</Dialog.Description>
        </div>
        ...
        <div hidden={confirmDiscard}><LLMProviderForm onDirtyChange={(value) => { dirtyRef.current = value; }} onSaved={() => { dirtyRef.current = false; setOpen(false); toast({ title: 'Đã lưu cấu hình LLM', description: 'Nhà cung cấp LLM đã được cập nhật.', variant: 'success' }); }} /></div>
