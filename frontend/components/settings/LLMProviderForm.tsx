...
export function LLMProviderForm({ onSaved, onDirtyChange }: { onSaved?: () => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const [provider, setProvider] = useState<LLMConfigInput['provider']>('openrouter');
  ...
  useEffect(() => {
    ...
    getLLMSettings(controller.signal).then(({ config }) => {
      const next = config ? { provider: config.provider, baseUrl: config.baseUrl, model: config.model } : { provider: 'openrouter' as const, baseUrl: LLM_PROVIDER_PRESETS.openrouter, model: '' };
  ...
  return <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void submit('save'); }}>
    {loadError && <div role="alert" className="rounded-compact border border-error/30 px-3 py-2 text-control text-error">Không thể tải cấu hình LLM. <Button type="button" variant="ghost" size="sm" onClick={() => setLoadVersion((value) => value + 1)}>Thử lại</Button></div>}
    ...
    <Input label="URL cơ sở" value={baseUrl} onChange={(event) => { setBaseUrl(event.target.value); onDirtyChange?.(true); }} readOnly={cloud} disabled={Boolean(busy) || loadError} required />
    {cloud && <p className="-mt-3 text-metadata text-text-muted">URL chính thức được DocAI quản lý tự động.</p>}
    ...
    <Input label="Khóa API" type="password" value={apiKey} onChange={(event) => { setApiKey(event.target.value); onDirtyChange?.(true); }} placeholder={hasApiKey ? 'Để trống để giữ khóa đã lưu' : 'Nhập khóa API'} required={cloud && !hasApiKey} disabled={Boolean(busy) || loadError} autoComplete="off" />
