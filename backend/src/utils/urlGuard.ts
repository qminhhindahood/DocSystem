...
function normalizeHostPort(hostname: string, port: number): string {
  ...
  return `${net.isIPv6(normalized) ? `[${normalized}]` : normalized}:${port}`;
}
...
export async function validateProviderTarget(
  ...
  if (hasNonGlobal) {
    if (!LOCAL_PROVIDERS.has(provider)) {
      throw new Error('URL resolves to a non-global/private/special address - not allowed for this provider');
    }
    const entry = normalizeHostPort(hostname, port);
    if (!allowlist.includes(entry)) {
      throw new Error(`Provider URL "${baseUrl}" resolves to a non-global address and "${entry}" is not in LOCAL_LLM_HOST_ALLOWLIST`);
    }
  }
...
export async function checkProviderUrl(
  baseUrl: string,
  allowLocalProvider: boolean,
): Promise<{ ok: boolean; reason?: string; resolvedHost?: string }> {
  try {
    await validateProviderTarget(baseUrl, allowLocalProvider ? 'lmstudio' : 'custom', []);
    return { ok: true };
  } catch (error) {
    return { ok: false, reason: error instanceof Error ? error.message : 'Invalid provider URL' };
  }
}

export function isAllowlistedLocalProvider(provider: string): boolean {
  return LOCAL_PROVIDERS.has(provider);
}

export type AllowlistedProvider = 'lmstudio' | 'ollama';
